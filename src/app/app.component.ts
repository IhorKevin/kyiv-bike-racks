    ...
    ngOnInit(): void {
        this.router.events
            ...
            .subscribe(route => {
                ...
                const rackTitle = (rack && !state) ? (rack.title || rack.owner_name || rack.street_address) : ''; // for initial loading only
                const pageTitle = route.snapshot.data.title;
                const currentTitle = stateTitle || rackTitle || pageTitle;
                const title = currentTitle ? `${currentTitle} | ${this.title}` : this.title;
